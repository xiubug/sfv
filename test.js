...
function check(path, value, def) {
	...
	var out = sfv(obj, path, def);
	assert.strictEqual(out, value, 'sfv(obj, "'+path+'") should be '+value+', got '+out);
	console.log(' ✓ sfv(obj, "'+path+'"'+ (def ? ', "'+def+'"' : '') + ')');

	if (path) {
		var arr = path.split('.');
		assert.strictEqual(sfv(obj, arr, def), value);
		console.log(' ✓ sfv(obj, ' + JSON.stringify(arr) + (def ? ', "'+def+'"' : '') + ')');
		console.log(' ✓ sfv(obj, '+JSON.stringify(arr)+')');
	}
}
...
assert.throws(sfv.bind(this, obj, undefined));
assert.throws(sfv.bind(this, obj, undefined, 'foo'));
